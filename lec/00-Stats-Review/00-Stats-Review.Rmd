...
### __Lectures:__
1. [Random variables](#section1). 
  - Sample vs. Population

2. [Probability density functions](#section2).
  - PDF, PMF, CDF
  - Mean and variance

3. [Correlation and independence](#section3)

4. [Types of distributions](#section4)

5. [Statistical inference](#section5)
  - Hypothesis testing, T-tests, p-values.

6. [Useful functions and properties](#section6)
...
#### We are starting with the concept of a *random variable* because most of what we're interested in is not perfectly constant in the real world
  - Wage, life expectancy, elasticity

Think about an MSU graduate's wages:
- Solid guess could be $45k per yr after graduation
- But surely not every graduate makes $45k per yr
- Maybe econ is $50k vs. $<$ insert oft-derided degree here $>$ at $40k
- Even knowing degree, still don't know wage perfectly!

$\rightarrow$ **Wage is a random variable when we look across graduates!**
--


So we can say "Alright, I don't know exactly what a graduate's wage will be, but what can we learn about it?"
...
But $\frac{1+4+36}{3} = 13.6667$ which is **not** $3^2$.
...
.pull-left[
```{r exDeviationsFromMean, include=T, echo=F, cache = T, fig.height=3}
...
knitr::kable(df, format='latex', align = 'cc') %>% 
  ...
  as_image(file = 'img/asdf.png', width = 3)
...
```{r spuriouschart}
include_graphics(copy_to_include(file.path(oldgraphics, 'SpuriousChart.png')))

# ![](img/SpuriousChart.png)
```
...
$$Cov(X,Y) = \frac{1}{N} \sum_{n=1}^N (x_n - \mu_X)(y_n - \mu_Y) = E(XY) - \mu_X \mu_Y$$
...
Example: **rolls of a fair die are .orange[iid]**
- The outcome of one roll tells us nothing about the value of the next roll
- The values are drawn from the same distribution
...
The $\sim$ is read as "is distributed as"
...
- $\bar{Y}$ is an estimator of the population mean $E(Y) = \mu_Y$
- The *sample variance*, $s^2$ from ealier is an estimator of $Var(Y)=\sigma^2$
...
- $Var(\bar{Y})$, so we know how much dispersion there is around $\mu$.
  - **IF** we know $\sigma^2_Y$.
...
```{r NormalExAfter, echo=F, include=T, out.width = '35%'}
par(mar=c(1,1,0,0)*2)
curve(dnorm, from=-4, to=4, xlab = 'y-bar', ylab = 'f(y-bar)')
abline(v = c(-1.96, 1.96))
```

<small>The vertical lines are where $F(z)=\{.025, .975\}$. The area under the curve between the lines is .95<small>
...
knitr::include_graphics(copy_to_include(file.path(oldgraphics, 'Table C-2.jpg')))
```
...
```{r NormalExAfter2, echo=F, include=T, out.width='45%'}
par(mar=c(1,1,0,0)*2)
...
include_graphics(copy_to_include(file.path(oldgraphics, 'Figure C-4.jpg')))
```
...
include_graphics(copy_to_include(file.path(oldgraphics, 'Table G-2.jpg')))
```
...
knitr::kable(df, format='latex', booktabs=T, align='c') %>%
  as_image(height=3, file='img/setupInferenceExampleTable.png')
```
]
...
include_graphics(copy_to_include(file.path(oldgraphics, 'Figure C-6.jpg')))
```
...
```{r plott, include=T, echo=F, out.width='60%', message=F}
...
```{r}
# We cover these in Single variable regression, incl. polys, natural log, elasticity, etc.
# DELETE if re-doing course
```
...
A linear function is one where the result is a linear transformation ( $ax + b$ ) of the inputs.
...
```{r lineExample1, echo=F, include=T, out.width='50%'}
...
```{r lineExample2, echo=F, include=T, out.width='50%'}
...
```{r naturalLog, include=T, echo=F, warning=F, out.width='50%'}
...
```{r exponentExample, include=T, echo=F, out.width='50%'}
...
**Now is the time to visit instructor office hours**. Some, maybe even a majority, can be perplexing - this is normal and expected.

--

Come to office hours and we can work through them until you're comfortable.
