...
- $n\sigma^2_x$ looks a lot like $SST_x$ since $\hat{\sigma}^2_x = \frac{1}{N-1}\sum (x_i - \bar{x})^2$
...
### Because, given $z_{1i},z_{2i},u_{1i},u_{2i}$, then $h_i$ and $w_i$ are .orange[completely determined]
...
### But estimating either equation by OLS would result in a biased $\alpha$. So we can't do it. 
...
$$(1-\alpha_2 \alpha_1)y_2 =  \beta_2 z_2 + \alpha_2\beta_1 z_1 + \underbrace{\color{red}{\alpha_2 u_1}}_{uh-oh} + u_2$$
This shows that $y_2$ is correlated with $u_1$. Estimating the $y_2$ equation from the previous slide gives a biased $\alpha_2$. .orange[This is simultaneity bias].
...
### We plugged in $y_1$ as a function of $y_2,z_1,u_1$ to estimate $\alpha_2$


$$(1-\alpha_2 \alpha_1)y_2 =  \beta_2 z_2 + \alpha_2\beta_1 z_1 + \alpha_2 u_1+ u_2$$
### and can do the same for $y_2$

$$(1-\alpha_2 \alpha_1)y_1 =  \beta_1 z_1 + \alpha_1\beta_2 z_2 + \alpha_1 u_2+ u_1$$
...
- Our estimation of $\pi_{21}$ and $\pi_{22}$ is unbiased - $z$'s are exogenous.
- $\pi_{21}$ and $\pi_{22}$ are functions of the structural parameters $\{\alpha_1,\alpha_2,\beta_1,\beta_2\}$
...
$$y_1 = \alpha_1 \color{red}{y_2} + \color{black}{\beta_1 z_1} + \color{red}{u_1}$$

$$y_2 = \alpha_2 \color{blue}{y_1} + \color{black}{\beta_2 z_2} + \color{blue}{u_2}$$
---
class: MSU
# Simultaneity
...
#### First stage
$$y_{it2} = \pi_{21} z_{i1} + \pi_{22} z_{i2} + \gamma^{1}_i + v_{i2}$$
#### Second stage
$$y_{it1} = \alpha_1 \hat{y}_{it2} + \beta_1 z_{i1} + \gamma^{2}_i + u_{i1}$$
